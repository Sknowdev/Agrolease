...
export async function getPriceSummary(
  ...
  // BUG FIX (2026-07-08): this used to filter with
  ...
  // page silently fell through to the "coming soon" message. Fixed by
  // dropping the absolute date filter and instead taking the most
  // recent `rowLimit` rows regardless of how old the latest one is. The
  // page itself is responsible for deciding how to present a stale
  // "latest" date (see PriceCard's "Updated: <date>" trust block) -
  // this function's job is just to not hide real data.
