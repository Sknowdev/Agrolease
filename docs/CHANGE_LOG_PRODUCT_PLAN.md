...
4. ~~The `0004_mobile_app_schema.sql` migration has NOT been run against the real Supabase project yet.~~ **RESOLVED 2026-07-20** — the founder ran `0004_mobile_app_schema.sql` and `0005_task2_auth_profile.sql` directly in the Supabase SQL Editor this session, using real credentials now present in this repo's `.env`. Verified afterward (service-role client): every new table exists, all 19 `country_config` rows and their existing columns are untouched, Nigeria's row has its 4 new mobile-app columns correctly populated. See `task_folder/task_app_progress.md`'s "Task 2 — Session Update 2026-07-20" for full detail.
...
## 15. Task 2 (2026-07-20) — country_config.active is Track B's flag, not a mobile-app signal; Profile ID format relaxed; Verification screen redesigned around Supabase's free-tier email limitation

**Plan says (Task-02-Auth-ProfileID.md):** `country_code` on new profiles is pulled from `country_config`, "never hardcoded" — no further detail on which column signals the active mobile-app country. Welcome's Profile ID is `user` + 4 digits, "uniqueness-checked, retry up to 10x," with inline `[edit]`.

**What was actually found and built:**
- **`country_config.active` is the public price website's own flag** (Track B — which countries have a live price page), confirmed directly against the live database: 17 of 19 rows have `active = true`, not just Nigeria. The original implementation queried on `active = true` and picked whichever row sorted first (Ghana, in practice) — a real bug, not a hypothetical one, confirmed by two real signups both landing on `country_code: GH` before the fix. Corrected to query `payment_provider IS NOT NULL` instead — the only column Task 1's migration actually populated for exactly Nigeria, making it the real, current mobile-app-specific signal. Documented directly in `backend/src/services/profileService.js`'s own comments, since this schema has no dedicated `mobile_app_active` column yet — one will be needed the moment a second mobile-app market launches.
- **Profile ID's *editable* format relaxed from `letters + exactly 4 digits` to a free-form 3-20 character username** (letters/numbers/underscores), per explicit founder instruction ("make the profile id as a unique username, not force number"). The *auto-generated* starting value (right after signup, shown at Welcome) still uses the brief's original `user####` pattern — only the user-editable validation changed. Case-insensitive uniqueness check is unchanged.
- **The Verification screen (Task 2 Step 5) was redesigned around a real, confirmed Supabase account-tier limitation**, not a design preference: this project's Supabase instance is on the free tier with no custom SMTP, so the "Confirm signup" email template is permanently locked to Supabase's default `{{ .ConfirmationURL }}` (a clickable link) and cannot be changed to `{{ .Token }}` (a numeric code) without a paid plan or a custom SMTP provider. The brief's literal wording ("Enter 6-digit code") assumed a code-based email, which this Supabase project can never send. Confirmed via the Admin API (`generateLink`/`verifyOtp`, using disposable test accounts deleted immediately after) that the underlying OTP mechanism does work if you had the code — the email itself simply never shows it to a real user on this tier. The screen was rebuilt as "Check your email" with a working Resend action and a manual "I've verified — Continue" check, rather than a code-entry field that could never be filled in correctly by a real user.

**Why this belongs in this log, not just the task tracker:** all three are real corrections to a stated plan/assumption, following this file's own rule — "never let a divergence go undocumented." None of them are cosmetic; all three were confirmed against the live database or the live Supabase Auth API before being changed, not guessed. Full session narrative (every bug, every fix, every still-open item): `task_folder/task_app_progress.md`'s "Task 2 — Session Update 2026-07-20" section.
...
## 18. Task 3 (2026-07-22) — `react-native-web`'s `Alert.alert` is a permanent no-op; password change has no old-password check of its own

**Plan says:** no planning document specifies how confirmations/error messages should be shown, or how password changes should be verified — both are implementation-level findings discovered through live usage, not deviations from a written spec.

**What was actually found:**
- **`react-native-web`'s `Alert.alert` implementation is `class Alert { static alert() {} }`** — confirmed directly in `node_modules/react-native-web/src/exports/Alert/index.js`. It does not fall back to `window.confirm`/`window.alert`, it does not log anything, it simply does nothing at all on web. Since this project's established testing method is `expo start --web` (per this file's own earlier notes), every `Alert.alert` call in the app had been silently non-functional on the one platform actually being tested, for the entire life of the project up to this point — this was the real root cause behind three separately-reported "bugs" (Delete Conduit not deleting, a Skip button doing nothing, login failures showing no error) that looked unrelated but shared one cause. Fixed with a new `lib/confirm.ts` (`confirmAction`/`notify`) that uses the real `Alert.alert` on native and `window.confirm`/`window.alert` on web — every `Alert.alert` call in the codebase has been replaced, confirmed via a repo-wide search returning zero remaining calls.
- **Supabase's `auth.updateUser({ password })` has no old/current-password parameter at all** — confirmed live (a disposable test account's password was successfully changed with zero verification of the previous one). It only requires a valid current session. Any "you must know your current password to change it" requirement has to be enforced by the calling application itself (re-authenticating via `signInWithPassword` first), not by Supabase's own API. Also confirmed live: **wrong password and non-existent email return the identical error** (`invalid_credentials`) from `signInWithPassword` — this is deliberate on Supabase's part, preventing login forms from being used to enumerate real accounts, and is not something this app can or should work around client-side.

**Why this belongs in this log:** both are genuine, confirmed-live platform-level findings that materially affect how auth/confirmation UI must be built for the rest of this project (not just Task 3) — following this file's own rule to document every divergence, not let it go unrecorded. Full session narrative: `task_folder/task_app_progress.md`'s "Task 3 — Session Update 2026-07-22" section. Itemized list of every other addition from this session: `task_folder/Task-03-Additions-Beyond-Spec.md`.


## 19. Task 3 (2026-07-22, end of session) — verification gap flagged, not a plan divergence: 2 fixes reported still failing live right after being verified in the same session

**Not a deviation from any planning document** — logged here only because `HANDOFF.md` directs readers to this file for anything that changes how the app actually behaves, and this genuinely does need visibility before Task 4 starts.

**What happened:** the header back-button fix and the profile-icon-tap-to-My-Profile fix (both built, code-reviewed, and `tsc`/`expo export`-verified earlier in the 2026-07-22 session — see Sections 17/18 above for the general pattern of real-bug-fixing this project follows) were reported by the founder, via live re-check, as still failing — in the same session, with no further code changes to either area in between the fix and the report.

**Why this is flagged as a likely verification gap, not a wrong fix:** both symptoms were already root-caused and fixed against confirmed live reproduction earlier in the exact same session. Nothing touched either code path again before the report came in. The most probable explanation is that the founder's browser/Metro bundle wasn't actually running the fixed code yet (stale cache, un-reloaded tab, or an old process) — but this is **not yet confirmed**, only the most likely explanation given the timeline.

**Not investigated or fixed this session** — the founder's session usage window ended as this was reported, with explicit instruction to stop and document rather than continue chasing it. Full detail: `task_folder/task_app_progress.md`'s "Task 3 — Session Update 2026-07-22 (continued again)" entry, and `task_folder/Task-03-Additions-Beyond-Spec.md`'s item 12.

**Also reported, same message:** the Conduit Workspace dashboard's arrangement (built later the same day, after Section 18 was written) still doesn't match the reference photo (`IMG_1365.jpeg`) closely enough, despite the card content being confirmed correct. No specifics given yet on what's different — needs a fresh, specific comparison with the founder before any further layout iteration, not a guess.
